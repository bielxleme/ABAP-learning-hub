    ...
    <header className="bg-[#1b2a4a] text-white border-b border-[#2d4373] shadow-md sticky top-0 z-50 w-full max-w-full overflow-x-hidden">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 flex flex-wrap items-center justify-between gap-2.5">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          {/* Real-time Update Icon: Always prominently shown when a new version is detected */}
          {updateInfo.hasUpdate ? (
            <button
              id="btn-live-update-pending"
              onClick={onOpenUpdates}
              title={`Nova versão v${updateInfo.latestVersion} pronta! Clique para atualizar agora.`}
              className="flex items-center space-x-1.5 px-2.5 py-1 bg-gradient-to-r from-emerald-500 via-teal-400 to-emerald-400 text-slate-950 font-black rounded text-xs shadow-lg transition-transform active:scale-95 cursor-pointer border border-emerald-300 ring-2 ring-emerald-400/40 animate-pulse"
            >
              <RefreshCw className="w-3.5 h-3.5 text-slate-950 animate-spin" style={{ animationDuration: '4s' }} />
              <span className="whitespace-nowrap">Atualizar App</span>
              <span className="w-2 h-2 rounded-full bg-emerald-950" />
            </button>
          ) : (
            onOpenUpdates && (
              <button
                id="btn-app-version-check"
                onClick={onOpenUpdates}
                title="Aplicativo Atualizado. Clique para verificar novidades e sincronização em tempo real."
                className="hidden md:flex items-center space-x-1 px-2 py-1 bg-blue-950/60 hover:bg-blue-900 text-blue-200 hover:text-white rounded text-[11px] font-mono border border-blue-600/30 cursor-pointer transition-colors"
              >
          ...
          {/* Windows 11 Desktop Installer Button */}
          {onOpenWindowsInstaller && (
            <button
              id="btn-windows-installer"
              onClick={onOpenWindowsInstaller}
              title="Instalador Nativo para Computador (Windows 11 / Desktop)"
              className="px-2 py-1 rounded text-slate-300 hover:text-sky-300 hover:bg-slate-700/60 transition-colors cursor-pointer hidden sm:flex items-center gap-1 text-xs border border-sky-600/30 bg-sky-950/40"
            >
